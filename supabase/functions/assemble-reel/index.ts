...
interface FFmpegServiceRequest {
  ...
  clips: { 
    url: string; 
    /** Timeline duration - FFmpeg should trim to this */
    requested_seconds: number;
    /** Provider duration - what was generated (for reference) */
    generated_seconds: number;
  }[];
 ...
 * Uses clip IDs, requested_seconds (timeline duration), transition settings, output settings, and voiceover.
 */
async function computeIdempotencyHash(
  ...
  const payload = JSON.stringify({
    // Use requested_seconds as the source of truth for hashing
    clips: clips.map(c => ({ id: c.id, requested_seconds: c.requested_seconds })),
 ...
 * Uses requested_seconds (timeline duration) as the source of truth.
 */
function safeTransitionDuration(clips: ClipInput[], requestedDuration: number): number {
  // Use requested_seconds for xfade calculation (timeline is truth)
  const minClipDuration = Math.min(...clips.map(c => c.requested_seconds));
...
Deno.serve(async (req) => {
  ...
  try {
      ...
      for (let i = 0; i < orderedClipIds.length; i++) {
        ...
        if (job?.output_url) {
          ...
          clips.push({
            ...
            requested_seconds,
            generated_seconds,
            duration: requested_seconds, // backwards compat
            index: i,
          });
        }
      }
    ...
    // Calculate expected duration using requested_seconds (timeline)
    const totalClipDuration = clips.reduce((sum, c) => sum + c.requested_seconds, 0);
    ...
    const ffmpegRequest: FFmpegServiceRequest = {
      job_id: jobId,
      idempotency_key: idempotencyKey,
      // Pass both durations - FFmpeg should trim to requested_seconds
      clips: clips.map(c => ({ 
        url: c.url, 
